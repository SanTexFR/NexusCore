...
class VarSerializer{
    ...
    public static @NotNull CompletableFuture<byte[]> serializeDataAsync(
        ...
        data.forEach((key, varEntry) -> {
            ...
            if (type.isWrapper()) {
                ...
                futures.add(valueFuture.thenApply(valueBytes -> {
    ...
    private static byte[] compress(byte[] input){
        try{
            try (ByteArrayOutputStream baos = new ByteArrayOutputStream();
                 LZ4BlockOutputStream lz4Out = new LZ4BlockOutputStream(baos, 64 * 1024, COMPRESSOR)) {

                // Optionnel : stocker la taille originale pour la décompression
                baos.write(ByteBuffer.allocate(4).putInt(input.length).array());

                lz4Out.write(input);
                lz4Out.finish(); // termine la compression
                return baos.toByteArray();
            }
        }catch (IOException e){
            e.printStackTrace();
    ...
    private static byte[] decompress(byte[] compressed){
        try{
            try (ByteArrayInputStream bais = new ByteArrayInputStream(compressed);
                ...
                // lire la taille originale si elle a été stockée
        ...
        return CompletableFuture.runAsync(() -> {
            ...
            while (buffer.hasRemaining()) {
                ...
                if (type == 0) {
                    ...
                    final VarSubType<Object> varType =
                    ...
                    CompletableFuture<Object> valueFuture =
                            varType.needAsync()
                                    ...
                                    varType.deserializeSync(valueByte)
                            );
                    ...
                    final VarSubType<Object> keyType =
                            (VarSubType<Object>) VarType.getTypes().get(keyTypeStr);
                    final VarSubType<Object> valueType =
                    ...
                    final MapVarType<Object, Object> mapVarType =
                            new MapVarType<>(mapType, keyType, valueType);

                    CompletableFuture<Map<Object, Object>> valueFuture =
                            mapVarType.needAsync()
                                    ? mapVarType.deserializeAsync(valueByte)
                                    : CompletableFuture.completedFuture(
                                    mapVarType.deserializeSync(valueByte)
                            );

                    futures.add(
                            valueFuture.thenAccept(map ->
